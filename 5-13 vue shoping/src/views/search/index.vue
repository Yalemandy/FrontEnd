<template>
  <div class="search">
    ...
    >
      <template #action>
        <div @click="search(useSearch)">搜索</div>
      </template>
    </van-search>
...
import { setHistoryList, getHistoryList } from "@/utils/storage.js";
export default {
  name: "SearchIndex",
  data() {
    return {
  ...
  methods: {
    ...
    search(value) {
      if (value) {
        const history = this.historyList.indexOf(value);
        if (history !== -1) {
          this.historyList.splice(history, 1);
        }
